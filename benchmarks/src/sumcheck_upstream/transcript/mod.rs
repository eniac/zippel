...
impl<F: PrimeField> IOPTranscript<F> {
    ...
    pub fn append_field_element(
        ...
        self.append_message(label, &to_bytes_local!(field_elem)?)
    }
    ...
    pub fn append_serializable_element<S: CanonicalSerialize>(
        ...
        self.append_message(label, &to_bytes_local!(group_elem)?)
    }
...
// Note: the upstream defined a `#[macro_export] to_bytes_local!` here, but
// `pari_upstream` already exports a sibling macro of the same name and
// `#[macro_export]` is whole-crate scope. The two call sites above use
// `to_bytes_local` instead — a module-scoped, non-exported equivalent.
macro_rules! to_bytes_local {
    ($x:expr) => {{
        let mut buf = ark_std::vec![];
        ark_serialize::CanonicalSerialize::serialize_compressed($x, &mut buf).map(|_| buf)
    }};
}
pub(crate) use to_bytes_local;
