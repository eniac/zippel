...
fn generate_proto(m: usize) -> String {
    ...
    format!(r#"fn eq_weights<G: Group, F: Scalar<G>>(instance x: [F; 1]) -> [F; 2] {{
    [(1 - x[0]), x[0]]
}}
...
// Recursive case: V ≥ 3 variables left. Fold the first variable by
// `round_challenge` to get a (V-1)-variable residual; marginalize the
// tail (V-2)-cube to produce the univariate round polynomial; sample
// at the points; recurse on `residual_poly`. Replaces the previous
// `marginalize(cfg)` builtin which is no longer in the language
// (`Op::Marginalize` was removed; only its private test-oracle copy
// remains in `backend/src/values.rs`).
fn sc_recurse_d3<G: Group, F: Scalar<G>, SC: Size, V: 3..SC + 1>(
    instance curr_poly:       Poly<F, V, 3>,
    instance points:          [F; 4],
    instance prev_challenges: [F; SC - V + 1],
    ...
    // `points` is `[0, 1, 2, ...]` as F values (built in the main
    // proto). Indexing `points[bit]` gives the F-typed 0 or 1 — avoids
    // `Fin * F` coercions which `F: Scalar<G>` doesn't allow.
    ...
    let evs_new = [round_poly(t) for t in points];
    evs <- evs_new;
...
// Base case: V = 2 variables left. Fold once to a univariate, sample
// it, draw the final challenge, return final_eval = g(r_final).
fn sc_recurse_d3<G: Group, F: Scalar<G>, SC: Size>(
    ...
    let residual_poly = eval(curr_poly, [round_challenge]);
    let evs_new = [residual_poly(t) for t in points];
    evs <- evs_new;
    verify(prev_eval == evs[0] + evs[1]);
    let g = interpolate(points, evs);
    r_final <- challenge<F>;
...
// Same shape as sc_recurse_d3 but for degree-2 round polynomials.
...
proto spartan<G: Group, F: Scalar<G>>(
    ...
    // Spartan proves R1CS satisfiability for the assignment z = w++io++[1]:
    //   (A·z) ⊙ (B·z) == C·z, element-wise.
    // The prover commits to the matrix-vector products az = A·z, bz = B·z,
    // cz = C·z; this clause states the gate-level R1CS constraint on those
    // vectors and binds z to the witness/instance. Linking az/bz/cz to
    // the matrices A/B/C encoded in mat_a_t/mat_b_t/mat_c_t requires
    // fully evaluating a multivariate MLE to a scalar — the current
    // `eval<R>` builtin requires a nonempty free range, and the parser
    // doesn't allow expression-applied-as-function (`(eval<0>(p,h))(t)`)
    // — so that structural identity is not yet expressible here.
    let one = f_one;
    let z = w ++ io ++ [one];
    az * bz == cz
{{
    ...
    // Build pts3 as explicit F-valued [zero, one, two, three]. Under
    // `F: Scalar<G>` integer literals don't auto-coerce to F (Field
    // does, but Scalar<G> doesn't), so `[i for i in 0..4]` would have
    // type `[Fin; 4]` and fail wherever an F is expected.
    let two   = one + one;
    let three = two + one;
    let pts3  = [zero, one, two, three];
    // Round 0 of the degree-3 sumcheck. Don't fold g_poly with any
    // challenge (there is no previous round); just sum it over the
    // (m_lit - 1)-dim boolean cube tail to get the univariate round
    // polynomial, then sample at pts3.
    let round_poly_1_0 = reduce(+, [
        eval<0>(g_poly, tail)
        for tail in [
    ...
    let evs1_0_new = [round_poly_1_0(t) for t in pts3];
    evs1_0 <- evs1_0_new;
    ...
    // Recurse with g_poly (unfolded) and the just-drawn rx0 as the next
    // round_challenge; the first recursive call folds it.
    ...
    let pts2   = [zero, one, two];
    let round_poly_2_0 = reduce(+, [
        eval<0>(m_poly, tail)
        for tail in [
    ...
    let evs2_0_new = [round_poly_2_0(t) for t in pts2];
    evs2_0 <- evs2_0_new;
