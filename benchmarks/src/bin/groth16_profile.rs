...
fn main() {
    ...
    // limits parallelism, this should scale linearly with thread count.
    {
        ...
        eprintln!(
            "CONTROL: 3× single G1 MSM (size {}): {:?}",
            q.len(),
            tic.elapsed()
        );
    }
    eprintln!(
        "=== Groth16 prove stage profile (log_size={}, C={}) ===",
        args.log_size, n
    );
    ...
    let full_bi: Vec<<GitFr as PrimeField>::BigInt> =
        t.full_assignment.iter().map(|x| x.into_bigint()).collect();
    let wit_bi: Vec<<GitFr as PrimeField>::BigInt> = t
        .witness_assignment
        .iter()
        .map(|x| x.into_bigint())
        .collect();
    ...
    rayon::scope(|s| {
        s.spawn(|_| {
            ...
            per_msm
                .lock()
                .unwrap()
                .push((0, started, tic2.elapsed(), std::thread::current().id()));
        });
        s.spawn(|_| {
            let started = scope_start.elapsed();
            let tic2 = Instant::now();
            ...
            per_msm
                .lock()
                .unwrap()
                .push((1, started, tic2.elapsed(), std::thread::current().id()));
        });
        s.spawn(|_| {
            let started = scope_start.elapsed();
            let tic2 = Instant::now();
            ...
            per_msm
                .lock()
                .unwrap()
                .push((2, started, tic2.elapsed(), std::thread::current().id()));
        });
        s.spawn(|_| {
            let started = scope_start.elapsed();
            let tic2 = Instant::now();
            ...
            per_msm
                .lock()
                .unwrap()
                .push((3, started, tic2.elapsed(), std::thread::current().id()));
        });
        s.spawn(|_| {
            let started = scope_start.elapsed();
            let tic2 = Instant::now();
            ...
            per_msm
                .lock()
                .unwrap()
                .push((4, started, tic2.elapsed(), std::thread::current().id()));
        });
    });
    eprintln!(
        "  5 MSMs via rayon::scope:         {:>9.2?}",
        scope_start.elapsed()
    );
    let pm = per_msm.into_inner().unwrap();
    for (idx, started_after, dur, tid) in pm {
        eprintln!(
            "    spawn {} started+{:>8.2?}, took {:>9.2?}, tid={:?}",
            idx, started_after, dur, tid
        );
    }
    ...
    let inner_tids =
        std::sync::Mutex::new(std::collections::HashSet::<std::thread::ThreadId>::new());
    let tic = Instant::now();
    rayon::scope(|s| {
        for _ in 0..5 {
            s.spawn(|_| {
    ...
    eprintln!(
        "  par_iter inside scope spawn — distinct OS thread IDs: {} (took {:?})",
        s_set.len(),
        dur
    );
