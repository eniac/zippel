...
#[derive(Parser, Debug)]
#[command(about = "Side-by-side IPA timing: zippel vs. Bp2aryStep (alex-ozdemir) (Secp256k1)")]
struct Args {
...
fn main() {
    ...
    println!("=== IPA: zippel vs. Bp2aryStep (alex-ozdemir) (Secp256k1) ===");
    println!("vector size N = 2^S; both sides verify internally");
    println!();
    println!(
        "  S  | zippel prove  zippel verify | bp prove    bp verify  | prove ratio  verify ratio"
    );
    println!(
        "-----+-----------------------------+------------------------+--------------------------"
    );
    ...
    for &s in &ss {
        ...
        println!(
            "  {s:>2} | {:>11.2?}  {:>13.2?} | {:>9.2?} {:>11.2?} | {:>10.2}x  {:>11.2}x",
            zt.prove,
            zt.verify,
            nt.prove,
            nt.verify,
