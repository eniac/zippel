...
//! chain `w[i+1] = w[i] * w[i]` with a single instance output). Shared setup lives in
...
fn main() {
    ...
    println!("statement: BenchCircuit — squaring chain w[i+1]=w[i]*w[i], single instance output");
