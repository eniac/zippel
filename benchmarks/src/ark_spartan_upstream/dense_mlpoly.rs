...
impl<F: PrimeField> DensePolynomial<F> {
  ...
  // Ported: the vendored ark-0.3 `#[cfg(feature = "multicore")]` branch
  // used the old serial-generic API without the `<G>` parameter and
  // called `.commit(...).compress()` on a slice. That inner API doesn't
  // exist in the 0.6 rewrite. This is the corrected multicore branch:
  // same shape as the non-multicore fallback below, but with `par_iter`
  // over rows. Gated on our `multicore` feature (default-on) so that
  // outer row-parallelism matches what libspartan does.
  #[cfg(feature = "multicore")]
...
impl<G: CurveGroup> PolyEvalProof<G> {
  ...
  pub fn verify(
    ...
    let L_repr = L.iter().map(|x| x.into_bigint()).collect::<Vec<_>>();
    let C_LZ = <G as VariableBaseMSM>::msm_bigint(C_affine.as_ref(), L_repr.as_ref());
