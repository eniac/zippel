//! ark-spartan vendored from https://github.com/zhenfeizhang/ark-spartan
//! (originally forked from Microsoft's `libspartan`) and hand-ported
//! from arkworks 0.3 → 0.6 in place. See the diff against upstream
//! for what changed; the protocol itself is unmodified.
...
mod tests {
  ...
  // Post-port smoke test: run NIZK on the ark-curve25519 v0.6 curve we
  // will actually use in bench_all, on a small R1CS. Verifies that the
  // 0.3 → 0.6 upgrade didn't silently break correctness for the curve
  // that matters for the zippel-vs-ark-spartan comparison.
  #[test]
  fn check_nizk_curve25519() {
    use ark_curve25519::EdwardsProjective as C25519;
    let (num_cons, num_vars, num_inputs) = (128, 128, 5);
    let gens = NIZKGens::<C25519>::new(num_cons, num_vars, num_inputs);
    let (inst, vars, inputs) = Instance::produce_synthetic_r1cs(num_cons, num_vars, num_inputs);
    assert!(inst.is_sat(&vars, &inputs).unwrap());
    let mut pt = Transcript::new(b"ark_spartan_smoke");
    let proof = NIZK::prove(&inst, vars, &inputs, &gens, &mut pt);
    let mut vt = Transcript::new(b"ark_spartan_smoke");
    proof.verify(&inst, &inputs, &mut vt, &gens).expect("NIZK verify FAILED post-port");
  }
