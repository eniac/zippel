...
impl<G: CurveGroup> R1CSProof<G> {
  ...
  pub fn verify(
    ...
    let expected_claim_post_phase1 =
      (*comm_prod_Az_Bz_claims - *comm_Cz_claim).mul_bigint(taus_bound_rx.into_bigint());
    ...
    let scalars = vec![r_A.into_bigint(), r_B.into_bigint(), r_C.into_bigint()];
    ...
    let comm_claim_phase2 =
      <G as VariableBaseMSM>::msm_bigint(bases_affine.as_ref(), scalars.as_ref());
    ...
    let scalars = vec![
      (G::ScalarField::one() - ry[0]).into_bigint(),
      ry[0].into_bigint(),
    ];
    ...
    let comm_eval_Z_at_ry = <G as VariableBaseMSM>::msm_bigint(bases.as_ref(), scalars.as_ref());

    // perform the final check in the second sum-check protocol
    let (eval_A_r, eval_B_r, eval_C_r) = evals;
    let expected_claim_post_phase2 =
      comm_eval_Z_at_ry.mul_bigint((r_A * eval_A_r + r_B * eval_B_r + r_C * eval_C_r).into_bigint());
