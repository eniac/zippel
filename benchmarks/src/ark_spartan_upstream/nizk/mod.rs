...
impl<G: CurveGroup> KnowledgeProof<G> {
  ...
  pub fn verify(
    ...
    let rhs = C.mul_bigint(c.into_bigint()) + self.alpha;
...
impl<G: CurveGroup> EqualityProof<G> {
  ...
  pub fn prove(
    ...
    let alpha = gens_n.h.mul_bigint(r.into_bigint());
  ...
  pub fn verify(
    ...
    let rhs = {
      let C = *C1 - *C2;
      C.mul_bigint(c.into_bigint()) + self.alpha
    };

    let lhs = gens_n.h.mul_bigint(self.z.into_bigint());
...
// Ported to arkworks 0.6 CanonicalSerialize/Deserialize API:
// - `serialize`/`deserialize` methods removed; the trait now dispatches
//   through `serialize_with_mode`/`deserialize_with_mode` with a
//   `Compress` mode parameter.
// - `serialized_size(compress)` takes a mode argument.
// - `CanonicalDeserialize` now requires a `Valid` impl for post-deserialize
//   validity checks (curve-point membership, etc.). We just delegate to
//   the field/group `Valid` impls the workspace already provides.
impl<G: CurveGroup> CanonicalSerialize for ProductProof<G> {
  fn serialized_size(&self, mode: ark_serialize::Compress) -> usize {
    self.alpha.serialized_size(mode) * 3 + self.z[0].serialized_size(mode) * 5
  }
  ...
  fn serialize_with_mode<W: ark_serialize::Write>(
    ...
    self.alpha.serialize_with_mode(&mut writer, mode)?;
    self.beta.serialize_with_mode(&mut writer, mode)?;
    self.delta.serialize_with_mode(&mut writer, mode)?;
    for e in self.z.iter() {
      e.serialize_with_mode(&mut writer, mode)?;
    }
    Ok(())
  }
}

impl<G: CurveGroup> ark_serialize::Valid for ProductProof<G> {
...
impl<G: CurveGroup> CanonicalDeserialize for ProductProof<G> {
  fn deserialize_with_mode<R: ark_serialize::Read>(
    mut reader: R,
    compress: ark_serialize::Compress,
    validate: ark_serialize::Validate,
  ) -> Result<Self, SerializationError> {
    let alpha = G::deserialize_with_mode(&mut reader, compress, validate)?;
    let beta = G::deserialize_with_mode(&mut reader, compress, validate)?;
    let delta = G::deserialize_with_mode(&mut reader, compress, validate)?;
    let mut z = [G::ScalarField::zero(); 5];
    for e in z.iter_mut() {
      *e = G::ScalarField::deserialize_with_mode(&mut reader, compress, validate)?;
    }
    Ok(Self {
      alpha,
      beta,
...
impl<G: CurveGroup> ProductProof<G> {
  ...
  fn check_equality(
    ...
    let lhs = *P + X.mul_bigint(c.into_bigint());
...
impl<G: CurveGroup> DotProductProof<G> {
  ...
  pub fn verify(
    ...
    let mut result = Cx.mul_bigint(c.into_bigint()) + self.delta
    ...
    result &= Cy.mul_bigint(c.into_bigint()) + self.beta == dotproduct_z_a.commit(&self.z_beta, gens_1);
...
impl<G: CurveGroup> DotProductProofLog<G> {
  ...
  pub fn verify(
    ...
    let lhs = (Gamma_hat.mul_bigint(c_s.into_bigint()) + beta_s).mul_bigint(a_hat_s.into_bigint()) + delta_s;
    let rhs = (g_hat + gens.gens_1.G[0].mul_bigint(a_hat_s.into_bigint())).mul_bigint(z1_s.into_bigint())
      + gens.gens_1.h.mul_bigint(z2_s.into_bigint());
