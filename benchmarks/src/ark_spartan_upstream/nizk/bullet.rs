...
impl<G: CurveGroup> BulletReductionProof<G> {
  ...
  pub fn prove(
    ...
    while n != 1 {
      ...
      let L = <G as VariableBaseMSM>::msm_bigint(bases.as_ref(), scalars.as_ref());
      ...
      let R = <G as VariableBaseMSM>::msm_bigint(bases.as_ref(), scalars.as_ref());
      ...
      for i in 0..n {
        ...
        G_L[i] = G_L[i].mul_bigint(u_inv.into_bigint()) + G_R[i].mul_bigint(u.into_bigint());
      }
    ...
    let Gamma_hat =
      G[0].mul_bigint(a[0].into_bigint()) + Q.mul_bigint((a[0] * b[0]).into_bigint()) + H.mul_bigint(blind_fin.into_bigint());
  ...
  pub fn verify(
    ...
    let scalars = s.iter().map(|x| x.into_bigint()).collect::<Vec<_>>();

    let G_hat = <G as VariableBaseMSM>::msm_bigint(group_element.as_ref(), scalars.as_ref());
    ...
    let Gamma_hat = <G as VariableBaseMSM>::msm_bigint(bases.as_ref(), scalars.as_ref());
