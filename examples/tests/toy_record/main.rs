...
fn main() {
    ...
    let args = ZippelArgs::new(PathBuf::from("examples/tests/toy_record/toy_record.zippel"));
    ...
    let analysis_result = std::panic::catch_unwind(|| {
        let analysis_args =
            ZippelArgs::new(PathBuf::from("examples/tests/toy_record/toy_record.zippel"));
...
fn create_inputs() -> Ctx<Vid, Value<ArkBls12_381>> {
    ...
    Ctx::from_iter([(Vid("n".to_string()), n), (Vid("b".to_string()), b)])
}
