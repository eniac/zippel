//! Spartan-NIZK with **pre-computed `az` / `bz` / `cz`** passed in as
//! private inputs (vs `spartan_full`, which computes them inside the
//! proto via dense matrix-vector multiplication). At M = 8 the dense
//! MVM unrolls into ~3·N² = ~200k mul-nodes inside the prover graph
//! and dominates Rayon scheduling overhead; lifting it out cuts the
//! prover-side graph by roughly half. The matrices stay public (used by
//! `partial_a/b/c` inside SC #2 — that piece is still unavoidable
//! without backend changes; see the report at the bottom of `run()`).
//!
//! Soundness link between `az/bz/cz` and the public matrices is via the
//! standard Spartan SC #2: SC #2 verifies that v_a/v_b/v_c (which come
//! from SC #1 over mle(az/bz/cz)) equal Σ_y M_x(rx, y) · z(y), with the
//! final `(ra·M_a + rb·M_b + rc·M_c)·v_z` check at random ry. A prover
//! lying about az/bz/cz can't satisfy that check w.h.p. — same argument
//! as full Spartan, no extra commitments needed.
//!
...
fn run_one(m: usize, invalid: bool, manual_zippel: Option<&str>) -> RunResult {
    ...
    let zippel_path = if let Some(path) = manual_zippel {
        ...
        let tmp_dir = std::env::temp_dir().join("zippel_spartan_sparse");
        std::fs::create_dir_all(&tmp_dir).expect("create tmp dir");
        let p = tmp_dir.join(format!("spartan_sparse_m{m}.zippel"));
    ...
    if invalid {
        // Corrupt `az` (the prover's claimed A·z) — that's the
        // analogue of "wrong matrix entry" for the sparse-inputs
        // variant. The verifier's SC #2 closing check `e_y ==
        // (ra·M_a + rb·M_b + rc·M_c)·v_z` independently re-derives
        // M_a(rx, ry) from the public mat_a_t and ry, so a corrupted
        // az breaks the link and the proof fails — exactly the
        // soundness story `spartan_full` exercised by perturbing
        // mat_a.
...
// ===== Spartan-NIZK proto (M = {m_lit} hardcoded), sparse-inputs variant.
//
// `az` / `bz` / `cz` are PRIVATE inputs — the harness pre-computes
// A·z, B·z, C·z natively and feeds them in. The body skips the dense
// matrix-vector multiplication that `spartan_full` does inline (3·N²
// mul-nodes). Matrices are still public because `partial_a/b/c` (for
// SC #2) and the SC #2 final R1CS-shape check both need them; that's
// the residual O(N²) bottleneck — unavoidable without backend support
// for a sparse MLE PolyVariant. =====
proto spartan_sparse<G1: Group, G2: Group, GT: Pairing<G1, G2>, F: Scalar<G1, G2>>(
