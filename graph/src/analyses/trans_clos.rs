    ...
    fn trans_clos_op(
        ...
        match op {
            ...
            Op::Evaluate(p, xs, typ) => {
                let p = self.trans_clos_op(dag, p.get().clone(), seen);
                let xs = self.trans_clos_op(dag, xs.get().clone(), seen);
            ...
            Op::Marginalize(op) => {
                Op::Marginalize(mk::<C>(self.trans_clos_op(dag, op.get().clone(), seen)))
            }
