...
impl<F: Field, T: Monomial> GroebnerBasis<F, T> {
    ...
    /// delegating to the [`crate::analyses::groebner::arkgb_engine`] via
    /// [`Monomial::compute_gb`]. The returned basis is *reduced*.
    ...
    /// Replace `self` with its reduced Gröbner basis. Equivalent to
    /// `*self = self.clone().buchberger()`. Kept as a method for
    /// API stability with callers that consumed the legacy unreduced
    /// `buchberger()` output and then called `reduce_groebner_basis()`.
    pub fn reduce_groebner_basis(&mut self)
    where
        F: Copy + Send + Sync + 'static,
        T: Clone,
    {
        *self = self.clone().buchberger();
    }

    /// Convenience: compute the reduced Gröbner basis. Since
    /// [`Self::buchberger`] already returns a reduced basis (the engine's
    /// `compute_gb` does a final reduction pass), this is an alias.
