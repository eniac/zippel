...
impl<F: Field, V: Var, T: Monomial<V>> GroebnerBasis<F, V, T> {
    ...
    pub fn contains_poly(&self, poly: &SparsePolynomial<F, V, T>) -> bool {
        let poly = poly.clone();
        let rem = self.reduce(poly);
        println!("Reduced poly: {}", rem);
    ...
    pub fn map_vars<VV: Var, TT: Monomial<VV>, FF: Fn(V) -> VV>(self, f: &FF) -> GroebnerBasis<F, VV, TT> {
        GroebnerBasis::new(self.num_vars, 
            self.basis.into_iter().map(|p| p.map_vars(f)).collect())
    }

    /// Eliminate equations with variables such that v.eliminate() == true
    pub fn eliminate(&mut self) {
        self.basis.retain(|p| p.vars().iter().any(|v| !v.eliminate()));
    }
...
impl<F: Field, V: Var, T: Monomial<V>> fmt::Display for GroebnerBasis<F, V, T> {
