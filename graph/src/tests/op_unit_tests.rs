...
mod op_construction_tests {
    ...
    fn test_eval_construction() {
        ...
        match eval_op {
            Op::Evaluate(_, _, typ) => assert_eq!(typ, ATyp::scalar()),
...
mod op_additional_tests {
    ...
    fn test_op_concat_vecs() {
        let v1 = GOp::<C>::vec(vec![Op::value(&scalar::<C>(1)), Op::value(&scalar::<C>(2))]);
        let v2 = GOp::<C>::vec(vec![Op::value(&scalar::<C>(3))]);
        let typ = ATyp::Vec(Box::new(ATyp::scalar()), 3);
    ...
    fn test_op_div_by_zero_panics() {
        ...
        let result = std::panic::catch_unwind(|| Op::div(a, zero, typ));
