...
// use spongefish::{DuplexSpongeInterface, DefaultHash};
// use lang::id::Vid;
...
// pub fn run_prover<C: ArkConfig, H: DuplexSpongeInterface>(graph: TDag<C>, inputs: Ctx<Vid, Value<C>>) -> Vec<Value<C>> {
...
// pub fn run_verifier<C: ArkConfig, H: DuplexSpongeInterface>(graph: TDag<C>, inputs: Ctx<Vid, Value<C>>) -> Vec<Value<C>> {
//     let mutex_graph = MutexGraph::new::<H>(graph);
//     let arc_graph = Arc::new(mutex_graph);
//     let result = MutexGraph::run_graph(arc_graph, Arc::new(inputs));
//     result
...
//     let prover_mutex_graph = MutexGraph::new::<DefaultHash>(prover_tdag);
...
//     let verifier_mutex_graph = MutexGraph::new::<DefaultHash>(verifier_tdag);
