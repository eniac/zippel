#!/usr/bin/env bash
# Experiment 3: completeness-analysis sweep across all 30 protocols
# (Singular backend, 20-minute per-run timeout, 16GiB memory limit,
# both inline and no-inline variants per protocol, by default --
# inline_all's own defaults).
#
...
#   artifact/scripts/run_inline.sh --inline-only --timeout 60 --protocols schnorr,groth16
