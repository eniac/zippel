...
use crate::typ::{TTyp, Range, Size, TypeVars, RangeTraversal};
use crate::parser::*;
...
impl<N> Decl<N> {
    pub fn proto(name: Fid, typevars: TypeVars, args: Args<N>, relation: Exp<N>, body: Exp<N>) -> Self {
        let sig = Sig { name, typevars, args, ret: TTyp::bool() };
        let body = Body::Proto { relation, body };
        Decl { sig, body }
    }

    pub fn func(name: Fid, typevars: TypeVars, args: Args<N>, ret: TTyp<N>, body: Exp<N>) -> Self {
    ...
    fn from_pest(
        ...
        match pair.as_rule() {
            ...
            Rule::func_decl => {
                ...
                let ret = TTyp::from_pest(&mut inner)?;
                // Function's body
                let next = inner.next().ok_or(ConversionError::NoMatch)?;
                let body =
                    match next.as_rule() {
...
fn proto_parser() {
    ...
    assert_eq!(UDecl::from_pest(&mut pairs).unwrap(), UDecl::proto(
        ...
        Args(vec![Arg::public("a", TTyp::varstr("F"))]),
...
fn fn_parser1() {
    ...
    assert_eq!(UDecl::from_pest(&mut pairs).unwrap(), UDecl::func(
        ...
        Args(vec![Arg::private("a", TTyp::vec(TTyp::varstr("F"), Size::from("N")))]),
        TTyp::varstr("F"),
...
fn decls_parser() {
    ...
    assert_eq!(UDecls::from_pest(&mut pairs).unwrap(), Decls(vec![
        ...
        UDecl::func(
            ...
            Args(vec![Arg::public("a", TTyp::vec(TTyp::varstr("F"), Size::from("N")))]),
            TTyp::varstr("F"),
