...
fn lub_tid() {
    ...
    assert!(Tid::lub_equ(m.clone(), f.clone(), &ctx).is_err());
...
fn lub_typ() {
    ...
    assert!(CTyp::lub_equ(tm.clone(), tf.clone(), &ctx).is_err());
